...
1. **Implement a Closure SLA.** Stories with all tasks Closed must be moved to Closed within 24 hours. The chronic multi-week closure gap is the single biggest drag on SAFe metrics across 6.4, 6.5, and now 6.6.

2. **Document capacity changes.** Each iteration's capacity allocation should have a one-line justification, especially when changing between iterations (3.0 -> 2.0 -> 1.0).

3. **Refine the backlog.** 15 of 22 visible backlog items are unassigned and in New state. Items like #157728 (49 days) and #195284 (51 days) need to be either scheduled or archived. Conduct a backlog grooming session to reduce noise.

4. **Separate carryover from new work in IP iterations.** Track which SP are "inherited debt" vs "new IP work" to maintain clean velocity metrics.
