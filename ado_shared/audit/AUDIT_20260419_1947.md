...
3. **Configure team capacity for Iteration 7.2.** (Ramon/PDM or team lead, 20 min.) Assign per-day capacity to Vicsante and Teofilo. This one-time config raises Team Capacity from 0 to ≥ 50 in every future audit until off-cycles — the single highest-leverage rubric fix.
4. **Assign sub-iteration to the 12 PI7-parent User Stories.** (PO, 30 min.) Distribute #202059–#202071 across 7.2/7.3/7.4 so they count toward Iteration Planning on the right sprint.
5. **Document the Shared Services Project Exception.** Add a bullet under `Project Exceptions` in `ado_shared/CLAUDE.md` stating that Enabler dominance + absence of User Stories on this team's board is structural (cross-cutting services). This lets future audits annotate WIB findings accordingly.

### P2 — PI7.2 and beyond

6. **Adopt "no Grooming items in iteration" rule.** Grooming-state items belong on the backlog, not the iteration. Easy policy win.
7. **Consider one synthetic "team User Story" per iteration** that captures a Shared Services deliverable (e.g., "Monthly AA cost-comparison report produced"). This partially mitigates the -40 "no User Story" WIB penalty without changing the team's actual work model.
8. **Track Shared Services work that lands on product teams' boards.** Add a section to the team's retrospective covering Enablers delivered through AA, CH, LS, etc. — gives a truer delivery view than this team's board alone can show.
